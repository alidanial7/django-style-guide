...
from {{cookiecutter.project_slug}}.common.models import RandomModel
...
def test_parse_sqlite_unique_violation():
    error = IntegrityError("UNIQUE constraint failed: common_randommodel.start_date")
    parsed = parse_integrity_error(error, model=RandomModel)
    assert parsed.error_type == IntegrityErrorType.UNIQUE
    assert parsed.columns == ("start_date",)


def test_parse_sqlite_not_null_violation():
    error = IntegrityError("NOT NULL constraint failed: common_randommodel.start_date")
    parsed = parse_integrity_error(error, model=RandomModel)
...
def test_parse_postgres_unique_pgcode():
    error = _integrity_with_pgcode("23505", "Key (start_date)=(2020-01-01) already exists.")
    parsed = parse_integrity_error(error, model=RandomModel)
    assert parsed.error_type == IntegrityErrorType.UNIQUE
    assert parsed.columns == ("start_date",)
...
def test_map_unique_raises_field_validation_error():
    ...
    with pytest.raises(ValidationError) as exc_info:
        map_integrity_error(error, model=RandomModel)
    assert "start_date" in exc_info.value.message_dict
    assert exc_info.value.error_dict["start_date"][0].code == ErrorCode.UNIQUE
...
def test_map_not_null_uses_not_null_code():
    ...
    with pytest.raises(ValidationError) as exc_info:
        map_integrity_error(error, model=RandomModel)
    assert exc_info.value.error_dict["start_date"][0].code == ErrorCode.NOT_NULL
